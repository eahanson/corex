defmodule CorexWeb.UserControllerTest do
  ...
  describe "create user" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post conn, user_path(conn, :create), user: Mom.user_attrs
  ...
  describe "update user" do
    ...
    test "redirects when data is valid", %{conn: conn, user: user} do
      conn = put conn, user_path(conn, :update, user), user: Mom.user_attrs |> Map.put(:email, "new@example.com")
    ...
    test "renders errors when data is invalid", %{conn: conn, user: user} do
      conn = put conn, user_path(conn, :update, user), user: Mom.user_attrs |> Map.put(:email, "bad")
      assert html_response(conn, 200) =~ "Edit User"
    end
  end
