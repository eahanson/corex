defmodule CorexWeb.WidgetViewTest do
  ...
  describe "widget %DetailTable{}" do
    setup do
      {:ok, user} = %{email: "email@example.com", password: "password"} |> Accounts.create_user()

      detail_table =
        DetailTable.new(title: "User Details")
    ...
    test "renders table action links", %{detail_table: detail_table} do
      ...
      assert_eq Enum.at(links, 0), ~r|<a class="detail-table__title-button" data-confirm="" href="/users/\d+/edit">Edit</a>|
      assert_eq Enum.at(links, 1), ~r|<a class="detail-table__title-button--delete" data-confirm="Really delete user \d+\?" data-csrf=".*" data-method="delete" data-to="/users/\d+" href="#" rel="nofollow">Delete</a>|
    end
